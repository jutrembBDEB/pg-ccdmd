...
sub getStatus {
  ...
  $self->{isNew} = $main::inputs_ref->{_next} || ($main::inputs_ref->{submitAnswers_2} &&
     $main::inputs_ref->{submitAnswers_2} eq ($self->{nextLabel} || "Go on to Next Part"));
  if ($self->{isNew}) {
    $self->checkAnswers;
...
sub initAnswers {
  ...
  foreach my $id (split(/;/,$answers)) {
    ...
    $value ='';
#    if ($main::inputs_ref->{'submitAnswers_2'}){ $value = '' ; }
...
sub nextButton {
  ...
  my $par = shift; $par = ($par ? $main::PAR : '');
  $par . qq!<input type="submit" name="submitAnswers_2" value="$label" !
...
sub grader {
 ...
 $result->{msg} .= 'Ce problème est divisé en plusieurs parties.'
                .  '<br/>'.$space.'<small>Votre résultat pour cette tentative s\'applique à cette partie seulement;</small>'
				.  '<br/>'.$space.'<small>votre résultat au total correspond au cumulatif de l\'ensemble des parties.</small>' 
  ...
  $result->{msg} .= '<p><b>Vous ne pouvez pas changer votre réponse lorsque vous passez à la prochaine partie!</b>'
