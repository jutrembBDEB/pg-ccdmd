...
sub _contextPolynomialFactorsFr_init {PolynomialFactors::Init()}
...
$self->Error("L'addition est seulement permise entre monômes")
...
$self->Error("La soustraction est seulement permise entre monômes")
...
#  $self->Error("Coefficients must come before variables or factors")
	$self->Error("Les coefficients doivent être situés devant les variables ou les facteurs")
...
sub checkFactors {
 ...
 #     $self->Error("Each factor can appear only once (combine like factors)") unless $factor eq "0";
  #    $self->Error("Only one constant coefficient or negation is allowed (combine them)");
		$self->Error("Chaque facteur doit se retrouver qu'une seule fois (combinez les facteurs semblables)") unless $factor eq "0";
		$self->Error("Une seule constante est permise (combinez-les)");
    }
...
$self->Error("Vous pouvez utiliser '%s' seulement entre des coefficients et des variables ou entre des facteurs",$self->{bop});
}
...
sub checkPolynomial {
 ...
 $self->Error("Dans un polynôme, vous devez diviser seulement par des nombres")
  ...
  if ($l->{isPoly} && $l->{isPoly} != 2) {
	...
	$self->Error("Une seule constante ou fraction est permise (combinez-les)")
...
sub checkPolynomial {
 ...
 $self->Error("Les exposants doivent être des constantes dans un polynôme")
  ...
  $self->Error("Vous pouvez appliquer un exposant seulement à un terme ou un facteur unique")
...
sub checkStrict {
 ...
 $self->Error("Vous pouvez appliquer un exposant seulement à une variable ou à un facteur");
}
...
sub checkPolynomial {
 ...
 #   $self->Error("Only one factor or constant can be negated") if $op->{isPoly} == 4;
   $self->Error("La double négation n'est pas permise") if $op->{isPoly} == 2;
    $self->Error("Un seul facteur ou une seule constante peut être de signe négatif") if $op->{isPoly} == 4;
  }
