...
sub _contextInequalitiesFr_init {Inequalities::Init()}
...
sub Init {
  ...
  $context->operators->add(
     ...
     'et' => {precedence => .45, associateivity => 'left', type => 'bin', string => " et ",
	       TeX => '\hbox{ et }', class => 'Inequalities::BOP::and'},

     'ou' => {precedence => .4, associateivity => 'left', type => 'bin', string => " ou ",
	      TeX => '\hbox{ ou }', class => 'Inequalities::BOP::or'},
  );
...
sub display {
  ...
  my $op = $context->{operators}{'ou'};
  my ($and,$or,$le,$ge,$ne,$open,$close) = @{{
    string => [' et ',$op->{string} || ' ou ',' <= ',' >= ',' != ','(',')'],
    TeX =>    ['\hbox{ et }',$op->{TeX} || $op->{string} || '\hbox{ ou }',
               ' \le ',' \ge ',' \ne ','\left(','\right)'],
  }->{$method}};
