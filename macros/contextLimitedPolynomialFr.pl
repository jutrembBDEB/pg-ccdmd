...
sub _contextLimitedPolynomialFr_init {LimitedPolynomial::Init()}; # don't load it again
...
sub _check {
  ...
  $self->Error("Votre réponse ne ressemble pas à un polynôme");
}
...
sub checkExponents {
  ...
  if ($l->{exponents}) {
    ...
    foreach my $i (0..scalar(@{$exponents})-1) {
      $self->Error("Une variable doit apparaître qu'une seule fois dans chacun des termes d'un polynôme")
...
sub checkPowers {
  ...
  foreach my $n (keys(%{$r->{powers}})) {
    $self->Error("Un polynôme simplifié ne peut pas avoir plusieurs termes avec le même degré")
...
sub checkStrict {
  my $self = shift;
  $self->Error("Vous ne pouvez pas utiliser '%s' entre les constantes",$self->{bop});
}
...
sub checkPolynomial {
  my $self = shift;
  my ($l,$r) = ($self->{lop},$self->{rop});
  $self->Error("L'addition est permise seulement entre des monômes") if $r->{isPoly};
  $self->checkPowers;
}
...
sub checkPolynomial {
  my $self = shift;
  my ($l,$r) = ($self->{lop},$self->{rop});
  $self->Error("La soustraction est permise seulement entre des monômes") if $r->{isPoly};
  $self->checkPowers;
}
...
sub checkPolynomial {
  ...
  $self->Error("Les coefficients doivent précédés les variables dans un polynôme")
    if LimitedPolynomial::isConstant($r) && ($l->{isPower} || $l->class eq 'Variable');
  $self->Error("La multiplication doit être utilisée seulement entre des coefficients et des variables");
}

sub checkStrict {
  my $self = shift;
  $self->Error("Vous pouvez utiliser '%s' seulement entre des coefficients et des variables dans un polynôme",$self->{bop});
}
...
sub checkStrict {
  ...
  $self->Error("Vous pouvez utiliser '%s' seulement pour former des fractions numériques",$self->{bop}) if $self->{lop}->class eq 'BOP';
}
...
sub checkPolynomial {
  ...
  $self->Error("Les exposants doivent être des entiers positifs dans un polynôme")
    unless $n > 0 && $n == int($n);
  LimitedPolynomial::markPowers($l);
  $self->{exponents} = $l->{exponents}; delete $l->{exponents};
...
sub _check {
  ...
  $self->Error("Vous ne pouvez pas utiliser de valeur absolue dans un polynôme");
}
...
sub _check {
  ...
  $self->Error("La fonction '%s' peut seulement être utlisée avec des nombres",$self->{name});
}
