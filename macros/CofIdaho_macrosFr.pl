...
#-------------------------------------------------------------------------------
             ...
             if ($CorrectFactors!=$#factors+1) 
                {
	        $ans_hash->{score}=0;
                $ans_hash->setKeys( 'ans_message' =>"Assurez-vous que votre réponse est factorisée complètement ou est entrée dans le format requis.");
                }
	   }
...
#--------------Split off the numerator/denominator
        ...
        if ($ans_hash->{score}==0) 
           {
           $ans_hash->setKeys( 'ans_message' =>"Simplifiez votre réponse.");
           }
        }
