...
sub _contextLimitedPowersFr_init {}; # don't load it again
...
sub OnlyIntegers {
  my $context = (Value::isContext($_[0]) ? shift : Value->context);
  $context->operators->set(
    '^'  => {class => 'LimitedPowers::OnlyIntegers', message => "constantes entières", @_},
    '**' => {class => 'LimitedPowers::OnlyIntegers', message => "constantes entières",@_},
  );
}
...
sub OnlyNonNegativeIntegers {
  my $context = (Value::isContext($_[0]) ? shift : Value->context);
OnlyIntegers($context, minPower=>0, message=>"constantes entières non négatives", @_);
}

sub OnlyPositiveIntegers {
  my $context = (Value::isContext($_[0]) ? shift : Value->context);
  OnlyIntegers($context, minPower => 1, message => "constantes entières positives", @_);
}

sub OnlyNonTrivialPositiveIntegers {
  my $context = (Value::isContext($_[0]) ? shift : Value->context);
  OnlyIntegers($context, minPower=>2, message=>"constantes entières plus grandes que 1", @_);
}
...
our @OnlyIntegers = (
  '^'  => {class => 'LimitedPowers::OnlyIntegers', message => "constantes entières"},
  '**' => {class => 'LimitedPowers::OnlyIntegers', message => "constantes entières"},
);
our @OnlyNonNegativeIntegers = (
  '^'  => {class => 'LimitedPowers::OnlyIntegers', minPower => 0, message => "constantes entières non négatives"},
  '**' => {class => 'LimitedPowers::OnlyIntegers', minPower => 0, message => "constantes entières non négatives"},
);
our @OnlyPositiveIntegers = (
  '^'  => {class => 'LimitedPowers::OnlyIntegers', minPower => 1, message => "constantes entières positives"},
  '**' => {class => 'LimitedPowers::OnlyIntegers', minPower => 1, message => "constantes entières positives"},
);
our @OnlyNonTrivialPositiveIntegers = (
  '^'  => {class => 'LimitedPowers::OnlyIntegers', minPower => 2, message => "constantes entières plus grandes que 1"},
  '**' => {class => 'LimitedPowers::OnlyIntegers', minPower => 2, message => "constantes entières plus grandes que 1"},
);
...
sub _check {
  my $self = shift;
  $self->SUPER::_check(@_);
  $self->Error("On ne peut pas mettre un exposant à e") if $self->{lop}->string eq 'e';
}

sub _eval {
  my $self = shift;
  Value::cmp_Message("On ne peut pas mettre un exposant à e") if $_[0] - $e == 0;
  $self->SUPER::_eval(@_);
}
